...
function NewTabDropdown({
  ...
  return (
    ...
    >
      ...
      <button onClick={() => { onOpenMacros(); onClose(); }} className={menuItemCls}>
  ...
  return (
    <main className="h-full w-full bg-surface-sunken relative">
      {openTabs.map((tab) => {
        ...
        if (tab.sessionType === SessionType.Macros) {
          return (
            <div
              key={tab.id}
              className={clsx("absolute inset-0 overflow-hidden", isActive ? "z-10" : "z-0 hidden")}
        ...
        if (tab.sessionType === SessionType.NetConf && session) {
          return (
            <div key={tab.id} className={clsx("absolute inset-0 overflow-hidden", isActive ? "z-10" : "z-0 hidden")}>
        ...
        if (tab.sessionType === SessionType.Snmp && session) {
          return (
            <div key={tab.id} className={clsx("absolute inset-0 overflow-hidden", isActive ? "z-10" : "z-0 hidden")}>
        ...
        if (tab.sessionType === SessionType.NfsExplorer && session) {
          return (
            <div key={tab.id} className={clsx("absolute inset-0 overflow-hidden", isActive ? "z-10" : "z-0 hidden")}>
...
export default function App() {
  ...
  const handleOpenMacrosTab = useCallback(() => {
    const now = new Date().toISOString();
    const session: Session = {
      id: crypto.randomUUID(),
      name: "Macros",
      type: SessionType.Macros,
      group: "",
      tags: [],
      connection: { host: "localhost", port: 0 },
      createdAt: now,
