...
describe("WhatsNewPanel", () => {
  ...
  it("FT-H-08: renders when version has not been dismissed", () => {
    ...
    expect(screen.getByText("What's New in CrossTerm 2.0.6")).toBeInTheDocument();
  ...
  it("FT-H-08: dismiss button hides the panel", async () => {
    ...
    expect(screen.queryByText("What's New in CrossTerm 2.0.6")).not.toBeInTheDocument();
  });
  ...
  it("FT-H-08: 'Don't show again' persists to localStorage", async () => {
    ...
    expect(localStorage.getItem("crossterm:whats-new-dismissed")).toBe("2.0.6");
  });

  it("FT-H-08: does not render when version is already dismissed", () => {
    localStorage.setItem("crossterm:whats-new-dismissed", "2.0.6");
