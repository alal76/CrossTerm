...
- **e2e-tests has no vault bootstrap.** Every spec except `vault.spec.ts` assumes the app is already past the vault-unlock/create screen, but nothing in `playwright.config.ts` (no `globalSetup`) or `e2e/storage-state.json` (seeds `firstLaunchComplete` and theme, nothing vault-related) ever creates or unlocks one. On a genuinely fresh app (which is what CI always is), `VaultUnlock`'s full-screen `fixed inset-0 z-50` overlay never goes away, so every other spec's clicks land on the overlay instead of the app underneath. 8 of 23 e2e tests fail this way as of this writing; this has apparently never worked in CI (blocked earlier by `cargo-audit` until today). Real fix needs a Playwright global-setup that drives real vault creation once (or a backend-side pre-seeded vault fixture) and persists the resulting state for the other specs to reuse — not attempted this pass given the scope.
