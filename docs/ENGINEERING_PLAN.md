...
| **Total (as of protocol-breadth branch)** | **589 Rust + 723 frontend** | **All passing** |
...
- ~~Frontend coverage is well under the CI gate's threshold~~ **FIXED 2026-08-16.** Was 43.4% lines / 40.1% functions / 35.1% branches against a configured 60%/60%/50% minimum when this was first found — real gaps, not measurement error, with dozens of components at or near 0%. Closed by writing real tests (not padding) for ~50 previously-untested components across a dozen commits: every Cloud provider panel (Aws/Azure/Gcp/KubectlPanel), the plugin system, all the setup wizards (First Launch, Import, Folder Sync), KeyManager, WifiScanner, and many smaller ones. Final: **65.4% lines / 60.3% functions / 54.9% branches** — `npx vitest run --coverage` exits 0. `SshTerminalTab.tsx` (684 lines, the most complex remaining file — the primary SSH terminal component) and a handful of others (Snippets/, `AircrackPanel.tsx`, `PolicyPanel.tsx`, `TeamPanel.tsx`, `RemoteFileBrowser.tsx`, `SsoButton.tsx`) are still untested; the gate passes without them, so they're tracked as further hardening, not a blocker. This pass also caught two real, if minor, missing-i18n-key bugs (`sftp.upload`, `network.wifiAdvancedDetails` — both fall back to their raw key string) — documented via the tests rather than silently patched, since fixing translations was out of scope.
