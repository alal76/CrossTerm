...
**Status:** merged to `main` 2026-08-15 (protocol-breadth phases 0–5). A follow-up pass the same day fixed three usability items (§5 of ROADMAP.md: U-4, U-9, U-12) that a roadmap audit found were falsely marked resolved. A second follow-up pass, also 2026-08-15, found and fixed CI itself was red on `main` post-merge (`cargo audit` blocking the whole pipeline — see below) — all commits on `feat/protocol-breadth`, ahead of `main` again as of this writing. 589 Rust + 475 frontend tests, all passing. `npx tsc --noEmit` clean, `cargo clippy -- -D warnings` clean (both CI invocations), `cargo audit` clean (with documented interim ignores), `npm audit --audit-level=high` clean.

**CI recovery pass (2026-08-15):** the merge to `main` broke CI — `cargo audit` was failing at 12 RUSTSEC findings, which blocks every downstream step (frontend build, clippy, tests, installer builds, SBOM) since it's the first gate. Root-caused and fixed:
...
- `cargo clippy -- -D warnings` (the exact CI invocation) had never been checked locally this whole session; it was failing on 40 findings, fixed via `cargo clippy --fix` plus manual fixes for what it couldn't auto-apply (dead code, one merged if/else in `snmp::ber_integer`, a nested `format!`).
