...
- **Frontend coverage is well under the CI gate's threshold: 43.4% lines / 40.1% functions / 35.1% branches against a configured 60%/60%/50% minimum.** Also never reached before today (blocked behind the same missing-dist-stub issue as the Rust half, fixed above). Real coverage gaps, not measurement error — several components sit at 0% (`ScriptAssistant.tsx`, `SshTerminalTab.tsx` — 674 lines, `TabTileGrid.tsx`, `TerminalSearch.tsx`, `ShortcutOverlay.tsx` in Terminal/, most of Snippets/, `SsoButton.tsx`). Closing this needs real new test-writing effort across dozens of files, not a config tweak — not attempted this pass.
