...
- [x] Frontend test coverage ≥ 75% — **DONE, independently re-verified 2026-08-21**: real measured `npx vitest run --coverage` gives 79.11% lines (7036/9085 statements, 73.74% functions, 69.45% branches), comfortably clearing both the ≥75% target here and the CI gate's 60%/60%/50% thresholds. The stale "~45%" figure in the metrics table below predates this verification and is corrected there. Not yet at the ≥85% next-tier target (see table).
...
| Frontend test coverage | 79.11% lines (measured, 2026-08-21) | ≥ 75% | ≥ 85% |
